            ...
            <div class="relative px-6 lg:px-8">
                <div class="mx-auto max-w-3xl">
                    <div class="columns-1 space-y-3 py-16">
                         <div class="flex flex-col md:flex-row group items-center gap-4">
                            <div style="background-image: url('{{ asset('/images/' . $user->image_path) }}');" class="h-56 w-56 rounded-full bg-cover bg-no-repeat bg-center"></div>
                                 <div>
                                    <h3 class="text-2xl font-semibold leading-7 tracking-tight text-gray-900 text-center md:text-left">{{ $user->name }}</h3>
                                    <p class="text-sm md:text-base font-semibold leading-6 text-red-500 text-center md:text-left">{{ $user->genre ?? "N/A" }}</p>
                                </div>
